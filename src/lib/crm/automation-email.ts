...
import { sendMeteredCommercialEmail } from "@/lib/commercial/email-usage";
...
function getRecordValue(record: AutomationRecord, fieldPath: string): unknown {
  return fieldPath.split(".").reduce<unknown>((currentValue, field) => {
    ...
    return (currentValue as AutomationRecord)[field];
  }, record);
}

function renderTemplate(template: string, record: AutomationRecord): string {
  return template.replace(
    /\{\{\s*([a-zA-Z][a-zA-Z0-9_.]*)\s*\}\}/g,
    (_match, fieldPath: string) => {
      const value = getRecordValue(record, fieldPath);
...
async function getRelatedCustomerEmail(
  ...
  if (entityType === "lead" || entityType === "customer") {
    return getText(record.email);
  }

  const customerId = getText(record.customerId);
  ...
  const leadId = getText(record.leadId ?? record.sourceLeadId);
...
async function resolveRecipient(
  ...
  const { tenantId, entityType, record, action } = input;

  const recipient =
    action.recipientSource === "fixed"
      ? action.recipientEmail
      : action.recipientSource === "owner"
        ? getText(record.ownerEmail)
        : action.recipientSource === "record"
          ? getText(record.email ?? record.customerEmail)
          : await getRelatedCustomerEmail(tenantId, entityType, record);

  if (!recipient) {
    throw new Error("No fue posible determinar el correo destinatario.");
  }
...
export async function sendAutomationEmail(
  ...
  if (!tenant) {
    throw new Error("La empresa ya no existe.");
  }

  const companyEmail =
    typeof tenant.metadata?.email === "string" && tenant.metadata.email.trim()
      ? tenant.metadata.email.trim().toLowerCase()
      : null;

  const replyTo = input.action.replyTo ?? companyEmail;

  const recipient = await resolveRecipient(input);

  const environment = getEnvironment();

  const resendApiKey = environment.RESEND_API_KEY ?? process.env.RESEND_API_KEY;

  if (!resendApiKey) {
    throw new Error("RESEND_API_KEY no está configurado.");
  }

  const subject = renderTemplate(input.action.subject, input.record).trim();

  const message = renderTemplate(input.action.message, input.record).trim();

  if (!subject || !message) {
    throw new Error("El asunto y el mensaje del correo son obligatorios.");
  }

  const response = await sendMeteredCommercialEmail({
...
<div style="margin:0;background:#f8fafc;padding:32px 16px;font-family:Arial,sans-serif;color:#0f172a">
  <div style="margin:0 auto;max-width:640px;overflow:hidden;border:1px solid #e2e8f0;border-radius:20px;background:#ffffff">
    ...
    <div style="border-top:1px solid #e2e8f0;padding:18px 28px;font-size:12px;line-height:1.5;color:#64748b">
      Enviado por ${escapeHtml(tenant.name)} mediante Datara.
      ${
        replyTo
