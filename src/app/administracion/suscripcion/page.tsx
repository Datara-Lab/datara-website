...
import { getCRMIndustryTemplates } from "@/config/crm/industries";

import { getDataraProduct, isDataraProductKey } from "@/config/datara-products";

import {
  EmailUsageCard,
  StorageUsageCard,
} from "@/components/administracion/EmailUsageCard";
...
type SubscriptionData = {
  ...
  pendingCatalogItemIds: string[] | null;
...
export default function SubscriptionPage() {
  const [data, setData] = useState<SubscriptionResponse["data"]>();
  ...
  useEffect(() => {
    ...
    async function loadSubscription() {
      try {
        ...
        const response = await fetch("/api/administracion/suscripcion", {
          cache: "no-store",
        });
        ...
        if (!response.ok || !result.success || !result.data) {
          throw new Error(
            result.error ?? "No fue posible cargar la suscripción.",
          );
        }
  ...
  async function openBillingPortal() {
    try {
      ...
      const response = await fetch("/api/administracion/suscripcion/portal", {
        method: "POST",
      });

      const result = (await response.json()) as PortalResponse;

      if (!response.ok || !result.success || !result.data?.portalUrl) {
        throw new Error(
          result.error ?? "No fue posible abrir el portal de pagos.",
        );
      }
      ...
      setPortalError(
        portalRequestError instanceof Error
  ...
  async function cancelScheduledPlanChange() {
    ...
    try {
      ...
      const response = await fetch("/api/administracion/suscripcion/cambio", {
        method: "DELETE",
      });
      ...
      if (!response.ok || !result.success) {
        throw new Error(
          result.error ?? "No fue posible cancelar el cambio programado.",
        );
      }
  ...
  const emailUsage = data?.emailUsage ?? null;

  const storageUsage = data?.storageUsage ?? null;

  const product =
    purchase && isDataraProductKey(purchase.productKey)
      ? getDataraProduct(purchase.productKey)
      : null;

  const industryName = purchase
    ? (getCRMIndustryTemplates().find(
        (template) => template.id === purchase.industry,
      )?.name ?? purchase.industry)
    : null;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
            Consulta el estado de la contratación, la próxima renovación y los
            productos activos de tu empresa.
          </p>
        </div>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
            <section className="rounded-[28px] border border-slate-200 bg-white p-7 shadow-sm sm:p-8">
                ...
                <div>
                  ...
                  {industryName ? (
                    <p className="mt-2 text-sm font-semibold text-slate-500">
                      Configuración para {industryName}
                    </p>
                  ) : null}
                </div>
              ...
              {subscription.status === "past_due" ? (
                <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
                  ...
                  <p className="mt-1 text-sm leading-6 text-amber-800">
                    Stripe volverá a intentar el cobro. Tu empresa conserva
                    temporalmente el acceso durante el periodo de recuperación.
                  </p>
                </div>
              ) : null}
              ...
              subscription.status === "canceled" ? (
                <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 px-5 py-4">
                  ...
                  <p className="mt-1 text-sm leading-6 text-red-800">
                    La información de la empresa está conservada. Actualiza el
                    método de pago para recuperar el acceso.
                  </p>
                </div>
              ) : null}
              ...
              <div className="mt-8">
                ...
                {emailUsage ? <EmailUsageCard usage={emailUsage} /> : null}
            ...
            <aside className="h-fit rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="text-sm font-bold text-slate-900">
                    {purchase?.billingPeriod === "annual"
                      ? "Anual"
                      : purchase?.billingPeriod === "monthly"
                        ? "Mensual"
                        : subscription.provider === "datara"
                          ? "Demo"
                          : "No disponible"}
                  </span>
                </div>
              ...
              subscription.pendingCatalogItemIds &&
              subscription.pendingChangeAt ? (
                <div className="mt-6 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-4 text-sm leading-6 text-blue-900">
                  <p className="font-black">Cambio de plan programado</p>
                  ...
                  <p className="mt-1">
                    ...
                    {new Intl.DateTimeFormat("es-MX", {
                      ...
                      minute: "2-digit",
                    }).format(new Date(subscription.pendingChangeAt))}
                    .
                  </p>

                  <p className="mt-1 text-xs font-semibold text-blue-700">
                    Nueva facturación:{" "}
                    {subscription.pendingBillingPeriod === "annual"
                      ? "anual"
                      : "mensual"}
                    {" · "}
                    {subscription.pendingCatalogItemIds.length} opciones
                    comerciales
                  </p>
              ...
              {subscription.provider === "stripe" && purchase ? (
                <Link
