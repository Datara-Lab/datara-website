...
export default function ClientesPage() {
  ...
  const configuredCustomersModule =
    getModule("contacts");
  ...
  async function handleDrawerSubmit(
    ...
    try {
      ...
      const payload =
        (await response.json()) as CustomerWriteResponse;
      ...
      <div className="mx-auto max-w-7xl">
        <section className="rounded-[32px] border border-amber-200 bg-white p-8 text-center shadow-sm sm:p-12">
          ...
          <p className="mx-auto mt-5 max-w-xl text-base leading-7 text-slate-500 sm:text-lg">
            El módulo de clientes no está habilitado para esta empresa.
          </p>
        </section>
      </div>
    );
    ...
    <>
      <div className="mx-auto max-w-7xl">
        <PageHeader
          eyebrow="Comercial"
          title={
            customersModule.pluralLabel
          }
          description={
            customersModule.description ??
            "Administra las personas y empresas que mantienen una relación comercial con tu negocio."
          }
          action={
